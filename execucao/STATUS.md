# Estado de execução
Use TODO, IN_PROGRESS, BLOCKED, REVIEW ou DONE. Evidência é obrigatória para DONE.

| ID | Estado | Evidência / observação |
| --- | --- | --- |
| T001 | DONE | Revisão independente aprovada |
| T002 | DONE | Revisão independente aprovada |
| T003 | DONE | Revisão independente aprovada |
| T004 | DONE | Revisão independente aprovada |
| T005 | DONE | Revisão independente aprovada |
| T006 | DONE | Revisão independente aprovada |
| T007 | DONE | Revisão independente aprovada |
| T008 | DONE | Revisão independente aprovada |
| T009 | DONE | Revisão independente aprovada |
| T010 | DONE | Revisão independente aprovada |
| T011 | DONE | Revisão independente aprovada |
| T012 | DONE | Revisão independente aprovada |
| T013 | DONE | Revisão independente aprovada |
| T014 | DONE | Revisão independente aprovada |
| T015 | DONE | CI 35955159394 PASS; revisão independente 35955252488 PASS |
| T016 | DONE | CI 35955830871 PASS; 133 testes + 4 Playwright; revisão independente aprovada |
| T017 | REVIEW | Visual V3/Lab em validação; CI técnico anterior verde; pendente rodada manual humana final |
| T018 | DONE | CI 36018449319 PASS; 139 testes + 4 Playwright; revisão independente aprovada; IDs derivados validados contra QuestProgressSchema |
| T019 | DONE | CI 36019365915 PASS; 149 testes + 4 Playwright; revisão independente aprovada; sim permanece sem dependência de protocol |
| T020 | DONE | CI 36020420310 PASS; 157 testes + 4 Playwright; revisão independente aprovada; collect/defeat com count e dedupe global validados |
| T021 | DONE | CI 36067908463 PASS; 165 testes + 4 Playwright; revisão independente aprovada; survive determinístico por tick validado |
